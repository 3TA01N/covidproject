...
log_path = os.path.join("/home/ubuntu/covidproject/backend/Logs/StockPull", datetime.now().strftime("%m-%d-%Y_%H:%M:%S")+ ".txt")
logging.basicConfig(filename=log_path,filemode='a',format='%(asctime)s,%(msecs)d %(name)s %(levelname)s %(message)s',datefmt='"%m/%d/%Y, %H:%M:%S"',level=logging.DEBUG)
logging.info("running stockPull.py")
try:
    ...
    def pull_stock_data(date1, date2):
            ...
            spy_data.to_csv(os.path.join("/home/ubuntu/covidproject/backend", 'SPYHistorical.csv'), index=False)
            spy_data.to_csv(os.path.join("/home/ubuntu/covidproject/backend", 'stockDataCSV.csv'), index=False)
